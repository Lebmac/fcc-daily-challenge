/* 
=== Left-Handed Seat at the Table ===
=== Source ===
https://www.freecodecamp.org/learn/daily-coding-challenge/2026-01-03

=== Functional / Input / Output ===
Given a 4x2 matrix (array of arrays) representing the seating arrangement for a meal, determine how many seats a left-handed person can sit at.

A left-handed person cannot sit where a right-handed person would be in the seat to the immediate left of them.
In the given matrix:

An "R" is a seat occupied by a right-handed person.
An "L" is a seat occupied by a left-handed person.
An "U" is an unoccupied seat.
Only unoccupied seats are available to sit at.
The seats in the top row are facing "down", and the seats in the bottom row are facing "up" (like a table), so left and right are relative to the seat's orientation.
Corner seats only have one seat next to them.
For example, in the given matrix:

[
  ["U", "R", "U", "L"],
  ["U", "R", "R", "R"]
]
The top-left seat is cannot be sat in because there's a right-handed person to the left. The other two open seats can be sat in because there isn't a right-handed person to the left.
*/

function findLeftHandedSeats(table) {
  let top = [...table[0], "U"];
  let btm = [...table[1].reverse(), "U"];
  let count = 0;

  for (var i = 0; i < table[0].length; i++) {
    count += checkSeatCriteria(top[i], top[i+1]);
    count += checkSeatCriteria(btm[i], btm[i+1]);
  }

  return count;
}

function checkSeatCriteria(seat, seatToLeft) {
  if (seat != "U") return 0;
  if (seatToLeft === "R") return 0;
  return 1;
}

/*
=== Design Rationale === 
1.  The input structure has two rows which need to be scanned in opposite directions.
    One row can be flipped for ease of scanning, the same logic can be applied to both rows.
2.  The logic will scan the rows and need some way of determining the last seat which has 
    fewer "availability" criteria. The arrays representing top and bottom can have an 
    "always empty seat appended to the array, this gives the programme something to compare
    to the end-of-row seats, so that custom logic is not required for this special case.
3.  The "available" criteria can be built into a standalone function, this prevents repeated
    code, and nicely classifies all the "available" criteria in one place, should it need
    to be adjusted in the future.
4.  The seat being inspected MUST have value "U"
    The only other criteria is that it does not have "R" to the left.
    All other cases it is fine to return "available" status.
*/
